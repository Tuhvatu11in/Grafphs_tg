...
def start():
    ...
    folder_name = 'folder_for_msg'  # Укажите название вашей папки
    folder_path = os.path.join(os.getcwd(), folder_name)  # Создаем путь к папке на основе текущей директории
    ...
    for file in os.listdir(folder_path):
        if file.startswith('messages') and file.endswith('.html'):
            message_files.append(folder_path + '/messages' + (str(n) if n > 1 else '') + '.html')
            n += 1
